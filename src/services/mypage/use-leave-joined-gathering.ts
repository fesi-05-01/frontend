...
interface CancelGatheringParams {
  gatheringId: number;
}
...
export function useCancelGathering() {
  ...
  return useMutation<JoinedGathering, ErrorResponseData, CancelGatheringParams>(
    {
      mutationFn: ({ gatheringId }) =>
        del<JoinedGathering>(`/gatherings/${gatheringId}/leave`),
      onSuccess: (_, { gatheringId }) => {
        toast.success('모임 참여를 성공적으로 취소했습니다.');
      ...
      onError: (error) => {
        switch (error.data.code) {
          case 'PAST_GATHERING':
            toast.error('이미 지난 모임은 취소할 수 없습니다.');
            break;
          case 'UNAUTHORIZED':
            toast.error('로그인이 필요합니다.');
            break;
          case 'NOT_FOUND':
            toast.error('해당 모임을 찾을 수 없습니다.');
            break;
          default:
