...
describe('GatheringCardLarge 컴포넌트', () => {
  ...
  describe('찜하기 기능', () => {
    ...
    it('이미 찜된 마감 모임은 찜 취소 후 버튼이 사라져야 함', async () => {
      ...
      // 버튼이 사라졌는지 확인
      expect(
        screen.queryByRole('button', { name: 'save-bye-large' }),
  ...
  describe('라우팅 동작', () => {
    it('카드 클릭 시 올바른 경로로 이동해야 함', async () => {
      ...
      const activeGathering = {
        ...MOCK_GATHERING,
        registrationEnd: '2025-12-31T23:59:59+09:00', // 미래 날짜로 확실하게 설정
      };

      render(<GatheringCardLarge gathering={activeGathering} />);

      const card = screen.getByRole('card-large');
  ...
  describe('버튼 상태 전환', () => {
    ...
    it('cardState가 ongoing일 때 JoinNowButton이 표시되어야 함', () => {
      ...
      render(<GatheringCardLarge gathering={ongoingGathering} />);
    ...
    it('cardState가 confirmation일 때도 JoinNowButton이 표시되어야 함', () => {
      ...
      render(<GatheringCardLarge gathering={confirmationGathering} />);
    ...
    it('오버레이 클릭 시 이벤트 전파가 중단되어야 함', async () => {
      ...
      render(
        <div onClick={mockCardClick}>
          <GatheringCardLarge gathering={closedGathering} />
        </div>,
      );
